...
namespace TestPartialOrdering_TestObjects
{
	...
	void TestPOP(const char* VLPPTEST_TESTFILE_ARGUMENT, const WString& description, vint componentCount, Func<void(List<vint>& items, Group<vint, vint>& groups)>&& callback)
	{
		TEST_CASE(description)
		{
			...
			pop.Sort();
			AssertPOP(pop, items, groups, componentCount);
		});
	}
...
TEST_FILE
{
	TEST_CASE(L"Test PartialOrderingProcessor without element")
	{
		PartialOrderingProcessor pop;
		pop.Sort();
